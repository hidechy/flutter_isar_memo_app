...
class MemoRepository {
  MemoRepository(this.isar, {this.sync = false}) {
  ...
  /// メモを追加する
  FutureOr<void> addMemo({required Category category, required String content}) {
    if (!isar.isOpen) {
      return Future<void>(() {});
    }
  ...
  /// メモを更新する
  FutureOr<void> updateMemo({required Memo memo, required Category category, required String content}) {
    if (!isar.isOpen) {
      return Future<void>(() {});
    }
